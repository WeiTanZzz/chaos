...
type QueryOf<C extends AnyCapability, P extends string> = { [K in keyof Omit<Body<C>, ParamKeys<P>>]: string | string[] }

type InputOf<C extends AnyCapability, M extends Method, P extends string> = M extends "get" | "delete"
...
type CapabilitySchema<C extends AnyCapability> = C extends { route: Route<infer M, infer P> }
    ? {
          [K in P]: {
              [K2 in `$${M}`]: {
